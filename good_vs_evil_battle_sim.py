...
#
#Thus, given the count of each of the races on the side of good, followed by the count of each of 
#the races on the side of evil, determine which side wins.
#Input:
#
...
def goodVsEvil(good, evil):
    #declare worth table variables
    good_worth = [1, 2, 3, 3, 4, 10]
    evil_worth = [1, 2, 2, 2, 3, 5, 10]
    evilsum = 0
    goodsum = 0
    #split inputs into lists of integers
    go = good.split(' ')
    g = list(map(int, go))
    ev = evil.split(' ')
    e = list(map(int, ev))
    ...
    gind = 0
    eind = 0
    #iterate over lists and multiply with worth values, comparing results and declaring a winner 
    for i in g:
        goodsum = goodsum + (i * good_worth[gind])
        gind = gind + 1
    for i in e:
        evilsum = evilsum + (i * evil_worth[eind])
        eind = eind + 1
    if evilsum < goodsum:
